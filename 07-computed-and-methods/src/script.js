Vue.createApp({
  ...
  methods: {
    discountPrice() {
      this.booksOfOReilly.forEach((element) => {
        const currentPrice = parseFloat(element.price.slice(1));
        const reducedPrice = currentPrice * 0.8;
        const finalPrice = "$" + reducedPrice.toFixed(2);
        element.discountPrice = finalPrice;
      });
    },
  },
}).mount("#app");
